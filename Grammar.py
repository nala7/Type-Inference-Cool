...
param_list, other_param, param, expr_list, let_var_list = G.NonTerminals(
    "<param-list> <other-param> <param> <expr-list> <let-var-list>"
)
branch_list, branch = G.NonTerminals("<branch-list> <branch>")
expr, comparer, arith, term, factor, atom = G.NonTerminals(
    "<expr> <comparer> <arith> <term> <factor> <atom>"
)
func_call, obj, arg_list, other_arg = G.NonTerminals(
    "<func-call> <obj> <arg-list> <other-arg>"
)
...
idx, num, new, strx, notx, isvoid, truex, falsex = G.Terminals(
    "id Int new String not isvoid true false"
)
...
def_attr %= (
    idx + colon + idx + left_arrow + expr + semi,
    lambda h, s: AttrDeclarationNode(s[1], s[3], s[5]),
)

def_func %= (
    idx + opar + param_list + cpar + colon + idx + ocur + expr + ccur + semi,
...
atom %= ifx + expr + thenx + expr + elsex + expr + fi, lambda h, s: ConditionalNode(
    s[2], s[4], s[6]
)
...
let_var_list %= (
    idx + colon + idx + comma + let_var_list,
    lambda h, s: [(s[1], s[3], None)] + s[5],
)
let_var_list %= (
    idx + colon + idx + left_arrow + expr + comma + let_var_list,
    lambda h, s: [(s[1], s[3], s[5])] + s[7],
)

branch_list %= idx + colon + idx + right_arrow + expr + semi, lambda h, s: [
    (s[1], s[3], s[5])
]
branch_list %= (
    idx + colon + idx + right_arrow + expr + semi + branch_list,
    lambda h, s: [(s[1], s[3], s[5])] + s[7],
)

func_call %= obj + dot + idx + opar + arg_list + cpar, lambda h, s: CallNode(
    s[1], s[3], s[5]
)
func_call %= idx + opar + arg_list + cpar, lambda h, s: CallNode(
    VariableNode("self"), s[1], s[3]
)
func_call %= obj + at + idx + dot + idx + opar + arg_list + cpar, lambda h, s: CallNode(
    s[1], s[5], s[7], s[3]
)

obj %= num, lambda h, s: ConstantNumNode(s[1])
obj %= idx, lambda h, s: VariableNode(s[1])
obj %= strx, lambda h, s: StringNode(s[1])
obj %= truex, lambda h, s: BoolNode(s[1])
obj %= falsex, lambda h, s: BoolNode(s[1])
obj %= opar + expr + cpar, lambda h, s: s[2]
obj %= new + idx, lambda h, s: InstantiateNode(s[2])
obj %= ifx + expr + thenx + expr + elsex + expr + fi, lambda h, s: ConditionalNode(
    s[2], s[4], s[6]
)
obj %= whilex + expr + loopx + expr + poolx, lambda h, s: LoopNode(s[2], s[4])
obj %= ocur + expr_list + ccur, lambda h, s: BlockNode(s[2])
obj %= casex + expr + ofx + branch_list + esacx, lambda h, s: CaseNode(s[2], s[4])
obj %= func_call, lambda h, s: s[1]
